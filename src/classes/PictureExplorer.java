...
public class PictureExplorer implements MouseMotionListener, ActionListener, MouseListener
{
  ...
  public ArrayList[] getArrayList() {
    return new ArrayList[]{rowList, colList};
  }
  ...
  private void displayPixelInformation(int pictureX, int pictureY)
  {
    ...
    if (isLocationInPicture(pictureX, pictureY))
    {
      ...
      rowList.add(rowIndex  + numberBase);
      colList.add(colIndex  + numberBase);
  ...
  public void actionPerformed(ActionEvent a)
  {
    ...
    if(a.getActionCommand().equals("5%"))
    {
      this.zoom(.05);
      enableZoomItems();
      five.setEnabled(false);
    }
    if(a.getActionCommand().equals("10%"))
    {
      this.zoom(.10);
      enableZoomItems();
      ten.setEnabled(false);
    }
    if(a.getActionCommand().equals("15%"))
    {
      this.zoom(.15);
      enableZoomItems();
  ...
  {
    Picture pix = new Picture("HEXAGON.jpg");
    Picture smallPix = pix.scale(.1, .1);
    smallPix.write("small.jpg");
    smallPix.explore();
  }
